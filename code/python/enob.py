...
class EnobCalculator:
    def __init__(self, filename, freq = 0, ):
        ...
        self.freq = float(os.path.basename(filename).replace(".txt", ""))
        self.lsb = 1
        self.n_bits = 12
        self.v_fs = 0.6
        self.v_max = 0.6
        ...
        self.h_data, self.l_data = file_reader(self.filename)
    ...
    def sine_fit(self, x, y, p0, bounds, verbose):
        try:
            parameters, covariance = curve_fit(self.sine_function, x, y, p0=p0,)
            A, F, P, C = parameters
    ...
    def do_fit(self, freq=0, adc = "h", verbose = False, graph = False):
        if adc == "l":
            print("Fitting sine wave of ADCL")
            y_data = self.l_data
        elif adc == "h":
            print("Fitting sine wave of ADCH")
            y_data = self.h_data
        else:
            print("Error with adc selection")
        ...
        phase_guess     = 3.14
        frequence_guess = self.freq * 0.001
        ...
        if graph:
            self.show_data(x_data, y_fit)
            self.show_data(x_data, y_data)
            self.show_data(x_data, y_guess)
            plt.show()
    ...
    def enob(self, adc = "h"):
        result, y_fit, y_data, p = self.do_fit(self.freq, adc)
